...
final class Curl extends AbstractCurl
{
    ...
    /**
     * RFC 6265: US-ASCII characters excluding `CTL`, whitespace DQUOTE, comma, semicolon, and backslash
     *
     * @var string[]
     */
    public const array RFC6265 = [
        // @formatter:off
        // %x21
        '!',
        // %x23-2B
    ...
    /**
     * Check if a given `$url` returns an `HTTP 200-300` status.
     *
     * Default options:
     * ```
     * CURLOPT_NOBODY         : true,
     ...
     * @param bool   $cached       Only perform the check once per session
     * @param mixed  $options
     *
     * @return bool
    ...
    public function download(
        ...
        if ( \is_callable( $location ) ) {
            ...
            $location = self::normalizePath( $location );

            $getFilename = \strrchr( $url, '/' );
            $setFilename = \strrchr( $location, '/' );

            if ( $getFilename !== $setFilename ) {
                if ( ! \str_contains( $setFilename, '.' ) ) {
            ...
            $this->downloadCompleteCallback = function( $instance, $fh ) use ( $tempFile, $filePath ) {
                // Close the open file handle before renaming the file.
                if ( \is_resource( $fh ) ) {
                    \fclose( $fh );
                }

                $this->copyFile( $tempFile, $filePath );
     ...
     */
    protected function getCurlErrorCodeConstants() : array
    {
     ...
     * @return string
     */
    protected function getCurlErrorCodeConstant() : string
    {
     ...
     * Get RFC 2616
     */
    protected function getRfc2616() : array
    {
     ...
     * Get RFC 6265
     */
    protected function getRfc6265() : array
    {
    ...
    {
        if ( \array_key_exists( __METHOD__, Curl::$deferredProperties ) ) {
            return Curl::$deferredProperties[__METHOD__];
        }
        ...
        $constants[CURLINFO_HEADER_OUT] ??= 'CURLINFO_HEADER_OUT';

        $constants = \array_flip( $constants );

        \ksort( $constants );
        return self::$deferredProperties[__METHOD__] = $constants;
    }
}
